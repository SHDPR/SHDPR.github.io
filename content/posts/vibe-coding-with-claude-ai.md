...
웹 개발을 배운 적이 없습니다. Next.js가 뭔지, Tailwind가 뭔지, Redis가 뭔지 몰랐습니다.
그런데 Claude AI와 대화만으로 이 블로그를 완성했습니다.
오늘은 그 과정을 솔직하게 공유합니다.

## 어떻게 시작했나

처음엔 막막했습니다. "블로그를 만들고 싶다"는 말 한 마디로 시작했습니다.
Claude가 기술 스택을 제안했고, 저는 그냥 승인했습니다.
...
**스크린샷을 적극 활용하세요** — "이상해 보여"보다 스크린샷 한 장이 훨씬 정확합니다. UI 문제는 말보다 보여주는 게 빠릅니다. 저는 거의 모든 UI 요청에 스크린샷을 첨부했습니다.

**원칙을 처음부터 정해두세요** — 저는 세 가지 원칙을 정했습니다: 항상 설계 먼저, 코딩 전에 승인 / 모르면 물어보기 / 모든 변경은 Git 커밋. 특히 탑다운 설계 원칙이 중요합니다. Claude가 코드를 짜기 전에 어떻게 구현할지 먼저 설명하게 하고, 마음에 들면 그때 진행시키는 방식입니다. 이 원칙은 대화가 바뀌어도 기억하고 따릅니다.

**해결책이 아니라 현상을 설명하세요** — "조회수 API를 SDK로 바꿔줘"가 아니라 "조회수가 안 올라가요"라고 하면 됩니다. 원인 분석과 해결책은 Claude가 찾습니다.

**마음에 안 들면 바로 말하세요** — 아바타를 다섯 번 다시 만들었습니다. Claude는 지치지 않습니다. 즉각적인 피드백이 가장 효율적입니다.

**배포 환경을 미리 알려주세요** — "Vercel에 올릴 거야"처럼 환경을 말해두면 그에 맞는 코드를 짜줍니다. 환경 차이로 생기는 버그를 미리 막을 수 있습니다.
